...
function setValueCircuito(element){
...
function getSubcircuitos(circuito)
{
	if (circuito != null && circuito.value != null && circuito.value != ''){
	  ...
	  {
	    url : Alfresco.constants.PROXY_URI + '/mx/com/gp/subcircuito?circuito=' + escape(circuito.value),
	    successCallback :
	    {
	      fn : function(response)
	      {
	      
	      console.log('response de subcircuitos' + fieldId);
	      
	        var txtObj = document.getElementById(fieldId + 'centroCostos');
	        
	        console.log('txtObj' + txtObj);
	        txtObj.innerHTML = "";

	        var opt;
	        var detalleValue = getValueSubcircuito();
	        ...
	        for (var i = 0; i < response.json.subcircuitos.length; i++)
	        {
	          opt = document.createElement('option');

	          opt.value = response.json.subcircuitos[i];
	          opt.text = opt.value;
	    ...
	    failureMessage : 'Error al cargar los Detalles para el Modulo [' + circuito.value + ']',
	    successMessage : 'Se cargaron los Detalles de [' + circuito.value + ']'
	  });
	 }
}
...
<div class="form-field">
   ...
   <#else>
      <label for="${fieldHtmlId}">${field.label?html}:<#if field.mandatory><span class="mandatory-indicator">${msg("form.required.fields.marker")}</span></#if></label>
         ...
         <select id="${fieldHtmlId}" name="${field.name}" tabindex="0" onchange="getSubcircuitos(this);"
